...
| `GET /.../projects/:projectId/tasks/:taskId/deliverable` | `view_deliverable` (the task's deliverable + full version history) |
| `POST /api/tasks/:taskId/deliverables` | `create_deliverable`; service re-checks that the caller works the task |
| `GET /api/deliverables/:deliverableId` · `/versions` · `/versions/:n` | `deliverableAccess` + `view_deliverable` |
| `GET /api/deliverables/:deliverableId/versions/:n/download` | `deliverableAccess` + `view_deliverable` (streamed, never a filesystem path) |
| `POST /api/deliverables/:deliverableId/versions` | `deliverableAccess` + `create_deliverable_version` (only after changes are requested) |
| `PATCH /api/deliverables/:deliverableId/versions/:n` | `deliverableAccess` + `upload_deliverable` (drafts only) |
| `PATCH /api/deliverables/:deliverableId/versions/:n/submit` | `deliverableAccess` + `submit_deliverable` (assignee/creator only) |
| `PATCH /api/deliverables/:deliverableId/versions/:n/review` | `deliverableAccess` + `review_deliverable` |
| `PATCH /api/deliverables/:deliverableId/versions/:n/approve` | `deliverableAccess` + `approve_deliverable`; self-review refused, all subtasks must be complete |
| `PATCH /api/deliverables/:deliverableId/versions/:n/request-changes` | `deliverableAccess` + `request_deliverable_changes`; self-review refused, feedback required |
