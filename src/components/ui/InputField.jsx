...
export function InputField({
  ...
  return (
    <div className={className}>
      ...
      <div className={`relative ${isPassword ? "has-password-toggle" : ""}`}>
        <input
          id={inputId}
          type={resolvedType}
          ...
          className={`h-11 w-full rounded-lg border bg-surface px-3.5 text-[15px] text-foreground placeholder:text-slate-400
            focus-ring transition-colors
            ${isPassword ? "pr-11" : ""}
            ${hasError ? "border-danger" : "border-border-subtle hover:border-slate-300 focus:border-primary"}`}
          {...props}
        />
        ...
        {isPassword && (
          <button
            ...
            className="absolute inset-y-0 right-0 flex w-11 items-center justify-center rounded-r-lg text-slate-400 transition-colors hover:text-foreground focus-ring"
          >
